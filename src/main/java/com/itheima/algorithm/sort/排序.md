...
- [归并排序](#归并排序)
  - [递归：自顶向下](#递归自顶向下)
  - [非递归实现：自底向上](#非递归实现自底向上)
  - [递归实现+元素少时插入排序](#递归实现元素少时插入排序)
...
| :--: | :--------: | :--------: | :--------: | :-----: | :--: | :--: | :----------------------------------------------------------: | :-:| :-:|
| 冒泡 |    O(n)    |  O($n^2$)  |  O($n^2$)  |  O(1)   |  Y   | 比较 |  最好情况需要优化版本，而且是已经有序的数组 | 慢| x记录最后一次交换位置|
| 选择 |  O($n^2$)  |  O($n^2$)  |  O($n^2$)  |  O(1)   |  N   | 比较 |交换次数一般少于冒泡                     | 慢|
|  堆  | O($nlogn$) | O($nlogn$) | O($nlogn$) |  O(1)   |  N   | 选择 |                                 | 快|非递归下潜|
| 插入 |    O(n)    |  O($n^2$)  |  O($n^2$)  |  O(1)   |  Y   | 比较 |最好情况是已经有序的数组                   | 慢|优化就是希尔排序|
| 希尔 |  O(nlogn)  |  O($n^2$)  | O($nlogn$) |  O(1)   |  N   | 插入 |                                 | 比堆排序还快| |
| 归并 | O($nlogn$) | O($nlogn$) | O($nlogn$) |  O(n)   |  Y   | 分治 |需要额外的O(n)的存储空间 |比希尔排序还快||
| 快速 | O($nlogn$) |  O($n^2$)  | O($nlogn$) | O(logn) |  N   | 分治 | 需要把枢轴值随机化来缓解最坏情况 ||
